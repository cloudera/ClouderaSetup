...
|ENV_TAGS
|Optional AWS resource tags to be applied to all deployed resources. Tags must be provided as comma-separated key=value pairs.
|Example: ENV_TAGS:{`owner=john,project=cdp-workshop,enddate=2026-12-31`}
|===
...
|To override the default value with current quota, kindly uncomment the corresponding entry in `configfile` and replace the override value e.g **7.3.2**
|===
